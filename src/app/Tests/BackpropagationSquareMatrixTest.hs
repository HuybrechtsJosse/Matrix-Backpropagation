...
import Numeric.LinearAlgebra.Static (Sq, R, vector, matrix, Sized (extract, konst), (#>), (<>), eye, (<.>), col)
...
testRunNet = TestCase (assertEqual "for (runNet input w b)"
                                   (matrix [0.004424859556066427,0.004424859556066427,0.004424859556066427,  0.0000816950831421474,0.0000816950831421474,0.0000816950831421474,  0.9954934453607913,0.9954934453607913,0.9954934453607913])
                                   (runNet input w b)
                               )

testNormSquare = TestCase (assertEqual "for (norm b)"
                                            (konst (-0.8199999999999998)) -- rounding error, normally -0.82
                                            (norm b)
                               )
...
testSquareError = TestCase (assertEqual "for (squareError output target)"
                                            (matrix [0.0518, 0.0518, 0.0518, 0.0518, 0.0518, 0.0518, 0.0518, 0.0518, 0.0518])
                                            (squareError output target)
                               )

testBackpropagationvectorMultiplication  = TestCase (assertEqual "for (backpropagation (tr (Var B) `times` Var B) net input target)"
                                          (N (konst 0) (2*b))
                                          (backpropagation (tr (Var B) `times` Var B) net input target)
                             )
testBackpropagationvectorMultiplication2  = TestCase (assertEqual "for (backpropagation (tr ((Var W) `times` (Var I)) `times` Var B) net input target)"
                                          (N (matrix [ 0.26,0.16899999999999998,0.52,  2.8999999999999995,1.885,5.799999999999999, -4.799999999999999,-3.1199999999999997,-9.599999999999998 ]) (matrix [-1.0200000000000002,-1.0200000000000002,-1.0200000000000002,  -6.332,-6.332,-6.332,  6.926,6.926,6.926]))
                                          (backpropagation (tr (Var W `times` Var I) `times` Var B) net input target)
                             )
testBackpropagationExp  = TestCase (assertEqual "for (backpropagation (tr (exp (Var B)) `times` exp (Var B)) net input target)"
                                          (N (konst 0) (2*exp b*exp b))
                                          (backpropagation (tr (exp (Var B)) `times` exp (Var B)) net input target)
                             )
testBackpropagationNorm  = TestCase (assertEqual "for (backpropagation (norm (exp (Var B))) net input target)"
                                          (N (konst 0) (exp b))
                                          (backpropagation (norm (exp (Var B))) net input target)
                             )
testBackpropagationDiv  = TestCase (assertEqual "for (backpropagation ((tr (exp (Var B)) `times` exp (Var B)) / norm (exp (Var B))) net input target)"
                                          (N (konst 0) (matrix [-0.2630616325903559,-0.2630616325903559,-0.2630616325903559,  3.865062663629692,3.865062663629692,3.865062663629692,  -0.055576860896900226,-0.055576860896900226,-0.055576860896900226]))
                                          (backpropagation ((tr (exp (Var B)) `times` exp (Var B)) / norm (exp (Var B))) net input target)
                             )
testBackpropagationSoftmax  = TestCase (assertEqual "for (backpropagation (tr (softmax (Var B)) `times` softmax (Var B)) net input target)"
                                          (N (konst 0) (matrix [-0.18176333598549885,-0.18176333598549885,-0.18176333598549885,  0.20254567788586553,0.20254567788586553,0.20254567788586553,  -0.020782341900366624,-0.020782341900366624,-0.020782341900366624]))
                                          (backpropagation (tr (softmax (Var B)) `times` softmax (Var B)) net input target)
                             )
testBackpropagation  = TestCase (assertEqual "for (backpropagation (netErr (Var I) (Var W) (Var B) (Var T)) net input target)"
                                          (N (matrix [-0.014642457828107816,-0.009517597588270081,-0.029284915656215632,  -0.00042861388302919967,-0.0002785990239689797,-0.0008572277660583993,  0.01507107171113701,0.00979619661223922,0.03014214342227402]) (matrix [-0.007321228914053908,-0.007321228914053908,-0.007321228914053908,  -2.1430694151459978e-4,-2.1430694151459978e-4,-2.1430694151459978e-4,  7.535535855568654e-3,7.535535855568654e-3,7.535535855568654e-3]))
                                          (backpropagation (netErr (Var I) (Var W) (Var B) (Var T)) net input target)
                             )
