...
testForwardADExp  = TestCase (assertEqual "for (forwardSparseGradient (const 2.45) (Exp (Times (Var X) (Var X))))"
                                          (SP (404.4386272426999::Double) (fromList [(X, 1981.7492734892296)]))
                                          (forwardSparseGradient (const 2.45) (exp (times (Var X) (Var X))))
                             )

testForwardADLog  = TestCase (assertEqual "for (forwardSparseGradient (const 2.45) (Log (Times (Var X) (Var X))))"
                                          (SP (1.7921760491132714::Double) (fromList [(X, 0.8163265306122448)]))
                                          (forwardSparseGradient (const 2.45) (log (times (Var X) (Var X))))
                             )

testForwardADDiv  = TestCase (assertEqual "for (forwardSparseGradient (const 2.45) (Div (Times (Var X) (Var X)) (Plus (Var X) One)))"
                                          (SP (1.7398550724637685::Double) (fromList [(X, 0.9159840369670238)]))
                                          (forwardSparseGradient (const 2.45) (Div (Times (Var X) (Var X)) (Plus (Var X) One)))
                             )

testForwardADNegate  = TestCase (assertEqual "for (forwardSparseGradient (const 2.45) (Negate (Times (Var X) (Var X))))"
                                          (SP (-6.0025::Double) (fromList [(X, -4.9)]))
                                          (forwardSparseGradient (const 2.45) (Negate (Times (Var X) (Var X))))
                             )
